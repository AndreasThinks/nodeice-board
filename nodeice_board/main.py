...
Usage:
    python main.py [--device_path=<path>] [--db_path=<path>] [--config_path=<path>] [--verbose]
...
def setup_logging(verbose: bool = False):
    """Configure logging for the whole application (console + log file)."""
    logging.basicConfig(
...
def kill_previous_instances():
    """Run the kill_previous_instances.sh script if it exists (repo checkouts only)."""
    repo_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    kill_script = os.path.join(repo_root, "kill_previous_instances.sh")

    if not os.path.isfile(kill_script):
        return
